--------------------------------------------------------
-- Minetest :: Medieval Flags Mod (flags)
--
...
local delete_if_orphan = function( self )
	...
	if node.name ~= "medieval_flags:upper_mast" and node.name ~= "ignore" then
...
minetest.register_entity( "medieval_flags:wavingflag", {
	initial_properties = {
		physical = false,
		visual = "mesh",
	...
	on_activate = function ( self, staticdata, dtime )
		...
		for o=1, #objs do
			...
			if lua and self ~= lua and lua.name == "medieval_flags:wavingflag" then
...
minetest.register_node("medieval_flags:lower_mast", {
...
local function spawn_flag( pos )
	...
	local obj = minetest.add_entity( flag_pos, "medieval_flags:wavingflag" )
...
minetest.register_node( "medieval_flags:upper_mast", {
	...
	paramtype2 = "facedir",
	tiles = { "flags_baremetal.png", "flags_baremetal.png" },
	wield_image = "mflags_pole_top_inv.png",
	inventory_image = "mflags_pole_top_inv.png",
	groups = { cracky = 2 },
	sounds = metal_sounds,
	is_ground_content = false,

	drop = {"medieval_flags:lower_mast 2", "medieval_flags:upper_mast"},
	...
	on_place = function( itemstack, placer, pointed_thing )
		...
		if (node.name ~= "medieval_flags:lower_mast" or node2.name ~= "medieval_flags:lower_mast") then
			return itemstack
		end
		...
		minetest.set_node(pointed_thing.under - vector.new(0, 1, 0), {name = "medieval_flags:upper_mast", param2 = param2 })
		minetest.set_node(pointed_thing.under, {name = "medieval_flags:upper_mast_hidden_1"})

		if not (minetest.is_creative_enabled(pn)) then
			itemstack:take_item()
		end

		local def = minetest.registered_nodes["medieval_flags:upper_mast"]
	...
	after_destruct = function( pos )
		...
		if minetest.get_node( above1 ).name == "medieval_flags:upper_mast_hidden_1" then
...
minetest.register_lbm({
	name = "medieval_flags:respawn_flags",
	label = "Respawn flags",
	nodenames = {"medieval_flags:upper_mast"},
...
minetest.register_lbm({
	name = "medieval_flags:update_node_meta",
...
if minetest.registered_items[flagpole_material] then
	minetest.register_craft({
		output = "medieval_flags:lower_mast 3",
		recipe = {
			{flagpole_material},
			{flagpole_material},
			{flagpole_material},
...
minetest.register_craft({
	output = "medieval_flags:upper_mast",
	recipe = {
...
-- or destructed.
minetest.register_node( "medieval_flags:upper_mast_hidden_1", {
	drawtype = "airlike",
	pointable = false,
	paramtype = "light",
...
flags.set_flag_at = function( pos, flag_name )
	...
	if node.name ~= "medieval_flags:upper_mast" then
		return false
	end
